...
def runBIDV(bankObject):
    ...
    for accountNumber in accountNumbers:
        ...
        # Append
        record = (
        d, bankObject.bank, accountNumber, termDays, term, irate, issueDate, expireDate, balance, interestAmount,
        currency)
        records.append(record)

    balanceTable = pd.DataFrame(
        data=records,
        columns=['Date', 'Bank', 'AccountNumber', 'TermDays', 'TermMonths', 'InterestRate', 'IssueDate', 'ExpireDate',
...
def runVTB(bankObject):
    ...
    downloadTable = pd.read_excel(
        join(bankObject.downloadFolder, downloadFile),
        names=['AccountNumber', 'TermMonths', 'InterestRate', 'Currency', 'IssueDate', 'ExpireDate', 'Balance',
               'InterestAmount'],
        skiprows=16,
        skipfooter=1,
        usecols='B,D:I,L',
    ...
    downloadTable[['IssueDate', 'ExpireDate']] = downloadTable[['IssueDate', 'ExpireDate']].applymap(
...
def runIVB(bankObject):
    ...
    for element in rowElements:
        ...
        records.append(
            (d, bankObject.bank, account, termDays, term, iRate, issueDate, expireDate, balance, interest, currency))
...
def runVCB(bankObject):
    ...
    for URL in URLs:
        ...
        termMonths = float(
            bankObject.wait.until(EC.presence_of_element_located((By.XPATH, xpath))).text.replace(' tháng', ''))
        # Interest rate
        xpath = '//*[@id="Lbl_LSFD" and text()!=""]'
        iRate = float(
            bankObject.wait.until(EC.presence_of_element_located((By.XPATH, xpath))).text.replace('% /năm', '')) / 100
        # Số dư, currency
        xpath = '//*[@id="Lbl_SDS" and text()!=""]'
        balanceString, currency = bankObject.wait.until(EC.presence_of_element_located((By.XPATH, xpath))).text.split()
        balance = float(balanceString.replace(',', ''))
        records.append(
            (d, bankObject.bank, account, termDays, termMonths, iRate, issueDate, expireDate, balance, iRate, currency))
...
def runFUBON(bankObject):
    ...
    startRow = \
    balanceTable.loc[balanceTable.iloc[:, 0].map(lambda x: isinstance(x, str) and 'Time Deposit Account' in x)].index[
        0] + 2
    endRow = \
    balanceTable.loc[balanceTable.iloc[:, 0].map(lambda x: isinstance(x, str) and 'Time Deposit Total' in x)].index[0]
...
def runFIRST(bankObject):
    ...
    for option in select.options:
        ...
        # Interest Amount
        downloadTable['InterestAmount'] = downloadTable['TermDays'] * downloadTable['InterestRate'] * downloadTable[
            'Balance'] / 365
        # Append
