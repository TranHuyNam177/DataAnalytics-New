...
def runSINOPAC(bankObject):
    ...
    checkData = bankObject.driver.find_elements(By.XPATH, xpath)
    if checkData:
        return pd.DataFrame()
...
def runVTB(bankObject):
    ...
    downloadTable = pd.read_excel(
        join(bankObject.downloadFolder, downloadFile),
        names=['ContractNumber','IssueDate','ExpireDate','TermMonths','Remaining','Currency','InterestRate','InterestAmount'],
        skiprows=18,
        usecols='B:E,G:J',
        dtype={
            'ContractNumber': object,
            'Remaining': np.float64,
            'InterestAmount': np.float64
        }
    )
    # IssueDate
    ...
    cols = ['Date','Bank','ContractNumber','TermDays','TermMonths','InterestRate','IssueDate','ExpireDate','Amount','Paid','Remaining','InterestAmount','Currency']
...
def runFIRST(bankObject):
    ...
    bankObject.driver.switch_to.frame('mainFrame')
    xpath = '//*[contains(text(),"Laon No.")]/following-sibling::td/*//select'  # chỗ này text trên IB để là Laon No.
    ...
    for account in accounts:
        ...
        downloadTable['Paid'] = 0
        # Interest Amount
        downloadTable['InterestAmount'] = downloadTable['TermDays'] * downloadTable['InterestRate'] / 365 * downloadTable['Amount']
        # Date
        if now.hour >= 8:
            d = now.replace(hour=0,minute=0,second=0,microsecond=0)  # chạy trong ngày -> xem là số ngày hôm nay
        else:
