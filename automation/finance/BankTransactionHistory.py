...
def runBIDV(bankObject,fromDate,toDate):
    ...
    transactionTable['Credit'] = transactionTable['Credit'].map(lambda x: float(x.replace(',','')) if isinstance(x,str) else x)
    # Balance
    transactionTable['Balance'] = transactionTable['Balance'].map(lambda x: float(x.replace(',','')) if isinstance(x,str) else x)
