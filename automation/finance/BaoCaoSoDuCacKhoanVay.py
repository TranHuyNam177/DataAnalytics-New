...
def run(  # chạy hàng ngày
    ...
    table = pd.read_sql(
        ...
        SELECT
            ...
            [t].[InterestAmount],
            [t].[Currency]
        FROM [BankLoanBalance] [t]
        WHERE [t].[Date] = '{t0_date}'
        ORDER BY [t].[Bank], [t].[ContractNumber]
        """,
        connect_DWH_CoSo,
    )
    ...
    file_name = f'Báo cáo số dư các khoản vay {file_date}.xlsx'
    ...
    sheet_title_name = 'LOAN BALANCE'
    sub_title_name = f'Date {file_date}'

    worksheet = workbook.add_worksheet(f'LoanBalance')
    ...
    worksheet.merge_range('C1:N1',CompanyName,company_name_format)
    worksheet.merge_range('C2:N2',CompanyAddress,company_info_format)
    worksheet.merge_range('C3:N3',CompanyPhoneNumber,company_info_format)
    worksheet.merge_range('A7:N7',sheet_title_name,sheet_title_format)
    worksheet.merge_range('A8:N8',sub_title_name,sub_title_date_format)
    ...
    worksheet.write_column('D11',table['ContractNumber'],text_center_format)
    ...
    worksheet.write_column('J11',table['Amount'],money_format)
    worksheet.write_column('K11',table['Paid'],money_format)
    worksheet.write_column('L11',table['Remaining'],money_format)
    worksheet.write_column('M11', table['InterestAmount'], money_format)
    worksheet.write_column('N11', table['Currency'], text_center_format)
