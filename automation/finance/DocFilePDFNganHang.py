...
def runENTIE(bank: str, month: int):
    ...
    for img in images:
        ...
        try:
            ...
            issueDate = dt.datetime.strptime(issueDateText, '%Y/%m/%d')
            expireDate = dt.datetime.strptime(expireDateText, '%Y/%m/%d')
            # extract giá trị termDays từ dataframe termDays
            termDaysText = dfTermDays.loc[dfTermDays.index[0], 'text']
            termDaysInImage = int(re.sub('[A-Z]', '', termDaysText))
            if (issueDate > expireDate) or ((expireDate - issueDate).days != termDaysInImage):
                continue
            # Term Days
            termDays = (expireDate - issueDate).days
            # Term Months
