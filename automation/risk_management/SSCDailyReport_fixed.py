...
def run(  # chạy hàng ngày sau batch giữa ngày
    ...
    detailTable = pd.read_sql(
        ...
        SELECT
            [f].MaCK,
            [f].[DuNoChoVayGDKQ]/1000000 [DuNoChoVayGDKQ],
            [f].[SoLuongCKChoVayCTCK]/1000 [SoLuongCKChoVayCTCK],
            [f].[KLNY]/1000 [KLNY],
            [f].[DuNo/VCSH] [DuNo/VCSH],
            [f].[VonChuSoHuuCTCK]/1000000 [VonChuSoHuuCTCK],
            [f].[CKChoVay/CKNY] [CKChoVay/CKNY],
            [f].[SanGiaoDich]
        FROM [FinalResult] [f]
        ORDER BY [MaCK]
        """,
    ...
    worksheet.write('M7', (equityPHS * 2)/1000000, number_bold_format)
