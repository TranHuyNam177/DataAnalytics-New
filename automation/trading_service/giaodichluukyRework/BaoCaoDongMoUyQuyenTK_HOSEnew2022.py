...
def run(
    ...
    # country code
    country_code = pd.read_excel(r'..\dependency\country_code.xlsx',index_col=0,squeeze=True)
    ...
    account_open = pd.read_sql(
        ...
        SELECT
            ...
            [a].[customer_name],
            CASE
                WHEN [a].[nationality] = N'Viet Nam' THEN 'VN'
                ELSE [a].[nationality]
            END [nationality],
            [a].[address],
    ...
    file_name = f'Danh sách KH đóng mở ủy quyền tài khoản PHS (2022) HOSE - từ {startDate} đến {endDate}.xlsx'
    ...
    if startMonth == endMonth and startYear == endYear:
        timePeriod = f'Tháng {convert_int(startMonth)} năm {startYear}'
    else:
        timePeriod = f'Tháng {convert_int(startMonth)} năm {startYear} tới Tháng {convert_int(endMonth)} năm {endYear}'
    note_title = f'({timePeriod})'
    ...
    sheet_motaikhoan.set_column('H:I',12)
    sheet_motaikhoan.set_column('J:J',14)
