...
def run(
    ...
    table_title = f'PHÍ CHUYỂN KHOẢN {timePeriod}'
    sheetName = timePeriod
    # Write to Báo cáo phí chuyển khoản
    file_name = f'Báo cáo phí chuyển khoản - từ {startDate} đến {endDate}.xlsx'
    ...
    file_name = f'Báo cáo phí chuyển khoản tổng hợp từ {startDate} đến {endDate}.xlsx'
    writer = pd.ExcelWriter(
        join(folder_name, dateRun, file_name),
        engine='xlsxwriter',
        engine_kwargs={'options':{'nan_inf_to_errors':True}}
