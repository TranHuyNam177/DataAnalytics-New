...
def run(
    ...
    table_title_hose = f'BẢNG KÊ GIÁ DỊCH VỤ GIAO DỊCH {timePeriod} - HOSE'
    # write to Bảng kê giá dịch vụ HOSE
    file_name_hose = f'Báo cáo giá dịch vụ giao dịch từ {startDate} đến {endDate} - HOSE.xlsx'
    writer = pd.ExcelWriter(
        join(folder_name, dateRun, file_name_hose),
        engine='xlsxwriter',
    ...
    table_title_hnx_upcom = f'BẢNG KÊ GIÁ DỊCH VỤ GIAO DỊCH {timePeriod} - HNX & UPCOM'

    # write to Bảng kê giá dịch vụ HNX & UPCOM
    file_name_hnx_upcom = f'Báo cáo giá dịch vụ giao dịch từ {startDate} đến {endDate} - HNX & UPCOM.xlsx'
    writer = pd.ExcelWriter(
        join(folder_name, dateRun, file_name_hnx_upcom),
        engine='xlsxwriter',
        engine_kwargs={'options': {'nan_inf_to_errors': True}}
    )
    workbook = writer.book
